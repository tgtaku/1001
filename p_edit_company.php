...
if(isset($_GET['name'])){
    ...
    $mysql_qry_user = "select * from assign_company_information_1 inner join users_information_1 on assign_company_information_1.users_id = users_information_1.users_id where assign_company_information_1.projects_id = '$project_id';";
    $result_user = mysqli_query($conn, $mysql_qry_user);
    if(mysqli_num_rows($result_user) > 0){
        $i = 0;
        while($row = mysqli_fetch_assoc($result_user)){
            $row_array_user[$i] = $row['users_name'];
            print_r($row_array_user[$i]);
            $i++;
        }
        for($i = 0; $i < count($row_array_user); $i++){
            $j = 0;
        $mysql_qry_com = "select * from users_information_1 inner join companies_information_1 on users_information_1.companies_id = companies_information_1.companies_id where users_information_1.users_name = '$row_array_user[$i]';";
