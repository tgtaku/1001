...
require "conn.php";
$mysql_qry_user = "select * from assign_company_information_1 inner join users_information_1 on assign_company_information_1.users_id = users_information_1.users_id where assign_company_information_1.projects_id = '$project_id' ORDER BY assign_company_information_1.companies_id ASC;";
...
if(mysqli_num_rows($result_user) > 0){
    ...
    while($row = mysqli_fetch_assoc($result_user)){
        ...
        $i = 0;
        for($p = 0; $p < count($row_array_user_db); $p++){
            $mysql_qry_com = "select * from companies_information_1 inner join users_information_1 on companies_information_1.companies_id = users_information_1.companies_id where users_information_1.users_name = '$row_array_user_db[$p]';";
            $result_com = mysqli_query($conn, $mysql_qry_com);
            if(mysqli_num_rows($result_com) > 0){
                
                while($row = mysqli_fetch_assoc($result_com)){
                    //print_r($row['companies_name']);
                    //print_r($row);
                        //$row_array_user_db[$i] = $row['users_name'];
                        $row_array_company_db[$i] = $row['companies_name'];
                        $i++;
